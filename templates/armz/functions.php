...
if (!defined('_ARTX_FUNCTIONS')) {
    ...
    /**
     * Decorates an article or block with the post style.
     *
    ...
    function artxBlock($caption, $content, $classes = '')
    {
        ...
        if (!$hasCaption && !$hasContent)
            return '';
                    artxFragmentBegin("<div class=\"block clearfix" . $classes . "\">");
            artxFragmentBegin("<div class=\"blockheader\"><h3 class=\"t\">");
            artxFragmentContent($caption);
            artxFragmentEnd("</h3></div>");
            artxFragmentBegin("<div class=\"blockcontent\">");
    ...
    function artxVMenuBlock($caption, $content, $classes = '')
{
    ...
    if (!$hasCaption && !$hasContent)
        return '';
                artxFragmentBegin("<div class=\"vmenublock clearfix" . $classes . "\">");
            artxFragmentBegin("<div class=\"vmenublockheader\"><h3 class=\"t\">");
            artxFragmentContent($caption);
            artxFragmentEnd("</h3></div>");
            artxFragmentBegin("<div class=\"vmenublockcontent\">");
            artxFragmentContent(artxPostprocessBlockContent($content));
            artxFragmentEnd("</div>");
     ...
     *  Select inputs with class != 'button'
     *   artxFindTags($html, array('input' => array('class' => '!button')))
     *  Select inputs with non-empty class
     *   artxFindTags($html, array('input' => array('class' => '!')))
     *  Select inputs with class != 'button' and non-empty class:
     *   artxFindTags($html, array('input' => array('class' => array('!button', '!'))))
     *  Select inputs with class = button but != 'button'
     *   artxFindTags($html, array('input' => array('class' => array('button', '!button'))))
     *
    ...
    function artxReplaceButtons($content, $filter = array('input' => array('class' => 'button')))
    {
        ...
        foreach (artxFindTags($content, $filter) as $tag) {
            if (is_string($tag))
                ...
                $tag['attributes']['class']['value'][] = 'button';
    ...
    function artxLinkButton($data = array())
    {
        return '<a class="' . (isset($data['classes']) && isset($data['classes']['a']) ? $data['classes']['a'] . ' ' : '')
            . 'button" href="' . $data['link'] . '">' . $data['content'] . '</a>';
    }
    ...
    function artxPostprocessContent($content)
    {
        ...
        $content = artxReplaceButtons($content, array('input' => array('class' => array('button', '!button')),
            'button' => array('class' => array('button', '!button')), 'button' => array('class' => array('btn', '!button'))));
        return $content;
    }
