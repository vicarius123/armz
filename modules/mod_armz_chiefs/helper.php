...
/**
 * Helper for mod_armz_chiefs
 *
 * @package     com_armz_chiefs
 * @subpackage  mod_armz_chiefs
 * @since       1.6
 */
class ModArmz_chiefsHelper
{
	/**
	...
	public static function renderElement($table_name, $field_name, $field_value)
	{
		...
		case 'pic':
						if (!empty($field_value)) {
							$result = JPATH_ADMINISTRATOR . 'components/com_armz_chiefs/images/chiefs/' . $field_value;
						} else {
							$result = $field_value;
						}
		break;
	...
	{
		return JText::_(
			'MOD_ARMZ_CHIEFS_HEADER_FIELD_' . str_replace('#__', '', strtoupper($params->get('table'))) . '_' . strtoupper($field)
		);
	}
